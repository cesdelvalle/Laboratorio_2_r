...
precip_delay <- flights %>%
  inner_join(weather, by = c("year", "month", "day", "hour", "origin"))%>% 
  group_by(precip) %>% 
  summarize(delay = replace_na(mean(dep_delay, na.rm = TRUE), 0))
...
ggplot(precip_delay, aes(x = precip, y = delay)) +
  ...
  labs(x = "Precipitation", y = "Delay", title = "Average Flight Delay by Precipitation") +
  ggtitle("Average Flight Delay by Precipitation") 
...
ggplot(flights_airports, aes(x = lon, y = lat, size = delay, color = delay)) +
  ...
  labs(x = "Longitude", y = "Latitude", title = "Flight Delays on May 10th, 2013")
